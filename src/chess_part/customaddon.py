...
def manually_set_pieces(board):
    ...
    Example input: "(c2,K,d3,P,d4,p)" to place pieces at the specified squares.
    """
    board.clear()  # Clear the board to start with an empty configuration
    print("Manually set up the board. Enter positions in the format '(square,piece,...)'.")
    print("Example: (c2,K,d3,P,d4,p) -> K at c2, P at d3, p at d4.")
    ...
    while True:
        user_input = input("Enter positions (or 'done' to finish): ").strip()
        ...
        try:
            ...
            # Ensure we have pairs of (square, piece)
            if len(positions) % 2 != 0:
                raise ValueError("Invalid input format. Must be pairs of square and piece.")
            
            for i in range(0, len(positions), 2):
                square = positions[i].strip()
                piece_symbol = positions[i + 1].strip()
